...
export function updateCueBarSpeed(cueIdx, barIdx, speed) {
  const cue = showConfig.cues[cueIdx];
  if (!cue?.bars?.[barIdx]) return;
  cue.bars[barIdx].speed = speed;
  saveShowConfig();
}

export function updateCueBarTime(cueIdx, barIdx, time) {
  const cue = showConfig.cues[cueIdx];
  if (!cue?.bars?.[barIdx]) return;
  cue.bars[barIdx].time = time || '';
  saveShowConfig();
  renderShowPage();
}
...
export function confirmAddBarToCue(cueIdx) {
  ...
  const timeSel  = document.getElementById(`cue_time_sel_${cueIdx}`);
  ...
  cue.bars.push({ barId, deadId, speed, time, flyperson: autoFP });
    ...
    if (cue.isNonFly) {
      ...
      const barLines = (cue.bars || []).map((cb, bi) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;padding:5px 14px;border-top:1px solid #0f172a;flex-wrap:wrap">
          ...
          ${cb.time
            ? `<span style="background:#0f172a;color:#93c5fd;border-radius:4px;padding:2px 8px;font-size:11px;font-weight:700;border:1px solid #1e40af">⏱ ${esc(cb.time)}</span>
               <button onclick="updateCueBarTime(${idx},${bi},'')" title="Clear time, use speed" style="background:none;border:none;color:#475569;cursor:pointer;font-size:11px;padding:0 2px;line-height:1">✕</button>`
            : `<select onchange="updateCueBarSpeed(${idx},${bi},this.value)"
                 style="background:#0f172a;color:#fbbf24;border:1px solid #27272a;border-radius:4px;padding:2px 5px;font-size:11px;font-weight:700;outline:none;cursor:pointer">
                 ${['V.Slow','Slow','Medium','Fast','Max'].map(s=>`<option ${s===cb.speed?'selected':''}>${s}</option>`).join('')}
               </select>
               <input type="text" value="" placeholder="⏱" title="Set a time instead of speed (e.g. 1:30)"
                 onblur="if(this.value.trim())updateCueBarTime(${idx},${bi},this.value.trim())"
                 style="background:transparent;border:none;border-bottom:1px dashed #1e40af;color:#93c5fd;padding:1px 3px;font-size:11px;font-weight:600;width:38px;outline:none">`}
      ...
      const addBarForm = addingBarToCue === idx
        ? `<div style="padding:8px 14px;background:#0a0f1a;border-top:1px solid #1e293b;display:flex;gap:6px;flex-wrap:wrap;align-items:center">
            ...
            <input id="cue_time_sel_${idx}" type="text" placeholder="⏱ time (opt)" title="Optional: set a specific time instead of speed"
              style="background:#0f172a;border:1px solid #1e40af44;border-radius:6px;color:#93c5fd;padding:5px 8px;font-size:12px;outline:none;width:90px">
